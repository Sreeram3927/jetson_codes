...
class FrameTransformation:
	...
	def offset_to_manipulatorFrame(self, x, y, z):
		"""
		Offset coordinates from the Robot Frame to the Manipulator Frame.
	...
	def transform_to_robotFrame(self, camera_point):
		"""
		camera_point = [Xc, Yc, Zc] in meters
		"""
	...
	def getTransformedCoordinates(self, camera_point):
		...
		x, y, z = self.offset_to_manipulatorFrame(
			x_rob,
			y_rob,
			z_rob
		)
	...
	def performHomography(self, u_pixel, v_pixel):
		...
		x_man, y_man, z_man = self.offset_to_manipulatorFrame(x_rob, y_rob, z_rob)
